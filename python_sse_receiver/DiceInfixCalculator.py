...
class DiceInfixCalculator:
    def __init__(self) -> None:
        ...
        self.precedence: dict[str, int] = {'+': 1, '-': 1, '*': 2, '/': 2, '%': 2, '^': 3, 'd': 4, 'P': 5, 'N': 5}
        # 定义右结合运算符，^ 和 d 是右结合的
        self.right_associative: set[str] = {'^', 'd', 'P', 'N'}
    ...
    def evaluate_postfix(self, postfix_tokens: List[Union[int, float, str]],
    ...
    def calculate(self, expression: str,
        ...
        # 3. 计算后缀表达式
        return self.evaluate_postfix(postfix, dice_details)


class TestDiceInfixCalculator(unittest.TestCase):
    def setUp(self):
        self.calculator = DiceInfixCalculator()
    ...
    def test_right_associative_exponentiation(self):
        """测试右结合指数运算: 2 ^ 3 ^ 2"""
        result = self.calculator.calculate("2 ^ 3 ^ 2")
    ...
    def test_float_multiplication(self):
        """测试浮点数乘法: 2.0 * 2"""
        result = self.calculator.calculate("2.0 * 2")
    ...
    def test_division_result_fraction(self):
        """测试除法结果为分数: 1 / 3"""
        result = self.calculator.calculate("1 / 3")
        self.assertAlmostEqual(1 / 3, result)

    def test_float_arithmetic_chain(self):
        """测试浮点数算术链: 1.0 / 3.0 * 3"""
        result = self.calculator.calculate("1.0 / 3.0 * 3")
        self.assertAlmostEqual(1.0, result)
    ...
    def test_exponentiation_with_parentheses(self):
        """测试带括号的指数运算: 2 ^ (0.5 * 2)"""
        result = self.calculator.calculate("2 ^ (0.5 * 2)")
        self.assertEqual(2.0, result)

    def test_single_d20_dice(self):
        """测试单个20面骰子: d20"""
        dice_info: List[DiceRollInfo] = []
        result = self.calculator.calculate("d20", dice_info)
    ...
    def test_two_d6_dice(self):
        """测试2个6面骰子: 2d6"""
        dice_info: List[DiceRollInfo] = []
        result = self.calculator.calculate("2d6", dice_info)
        self.assertIsInstance(result, (int, float))
    ...
    def test_dice_multiplication(self):
        """测试骰子乘法: 2 * d10"""
        dice_info: List[DiceRollInfo] = []
        result = self.calculator.calculate("2 * d10", dice_info)
        self.assertIsInstance(result, (int, float))
    ...
    def test_nested_dice_expression(self):
        """测试嵌套骰子表达式: (2d4)d6"""
        dice_info: List[DiceRollInfo] = []
        result = self.calculator.calculate("(2d4)d6", dice_info)
        self.assertIsInstance(result, (int, float))
        # 2d4的结果在2-8之间，表示骰子数量
    ...
    def test_dice_addition(self):
        """测试骰子加法: 3d6 + 2d8"""
        dice_info: List[DiceRollInfo] = []
        result = self.calculator.calculate("3d6 + 2d8", dice_info)
        self.assertIsInstance(result, (int, float))
    ...
    def test_calculated_dice_sides(self):
        """测试计算得出的骰子面数: 3d( 2 + 4 )"""
        dice_info: List[DiceRollInfo] = []
        result = self.calculator.calculate("3d( 2 + 4 )", dice_info)
        self.assertIsInstance(result, (int, float))
        # 等同于3d6，范围应在3-18之间
    ...
    def test_d_followed_by_operator(self):
        """测试d后面跟着运算符的情况: 2d+3"""
        dice_info: List[DiceRollInfo] = []
        result = self.calculator.calculate("2d+3", dice_info)
        self.assertIsInstance(result, (int, float))
    ...
    def test_complex_dice_expression(self):
        ...
        dice_info: List[DiceRollInfo] = []
        result = self.calculator.calculate("2d2d2", dice_info)
        self.assertIsInstance(result, (int, float))
        # 2d2范围: 2-4
